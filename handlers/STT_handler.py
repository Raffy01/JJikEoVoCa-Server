...
def handle(conn, addr, payload):
    ...
    try:
        ...
        safe_file_name, audio_buffer = utils.receive_file(conn, raw_filename, raw_filesize)
        audio_bytes = audio_buffer.getvalue()

        result = False
        try:
            inferred_text = STT_google.speech_to_text(audio_bytes, codec_mismatch_occured)
            ...
            converted_audio_bytes = utils.convert_to_wav_bytes(audio_bytes)

            inferred_text = STT_google.speech_to_text(converted_audio_bytes, codec_mismatch_occured)
...
def sendback(conn, addr, payload):
    ...
    try:
        ...
        try:
            inferred_text = STT_google.speech_to_text(audio_data, codec_mismatch_occured)
        except AttributeError:
            codec_mismatch_occured = True
            ...
            converted_audio_bytes = utils.convert_to_wav_bytes(audio_data)
